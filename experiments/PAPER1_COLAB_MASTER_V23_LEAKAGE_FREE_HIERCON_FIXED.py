...
if STAGE == "week1_audio":
    ...
    # ---- S2: leakage-free layer-wise probes -------------------------------
    # Train on source-train. Select layers using source-validation only.
    # Target scores are held out for final reporting and never drive selection.
    probe, bank_target, bank_validation = {}, {}, {}
    for mk in CFG.SSL_MODELS:
        Xtr = np.load(FE[(mk, "src_tr")][0]).astype(np.float32)
        ytr = np.load(FE[(mk, "src_tr")][1])
        Xva = np.load(FE[(mk, "src_va")][0]).astype(np.float32)
        yva = np.load(FE[(mk, "src_va")][1])
        Xte = np.load(FE[(mk, "tgt")][0]).astype(np.float32)
        yte = np.load(FE[(mk, "tgt")][1])
        probe[mk] = {}
        for L in range(Xtr.shape[1]):
            probe[mk][L] = {}
            for sd in CFG.SEEDS:
                set_seed(sd)
                scaler = StandardScaler().fit(Xtr[:, L])
                clf = LogisticRegression(max_iter=2000, random_state=sd, class_weight="balanced")
                clf.fit(scaler.transform(Xtr[:, L]), ytr)
                score_val = clf.predict_proba(scaler.transform(Xva[:, L]))[:, 1]
                score_tgt = clf.predict_proba(scaler.transform(Xte[:, L]))[:, 1]
                probe[mk][L][sd] = {
                    "validation_auc": float(roc_auc_score(yva, score_val)),
                    "target_auc": float(roc_auc_score(yte, score_tgt)),
                }
                bank_validation[(mk, L, sd)] = score_val
                bank_target[(mk, L, sd)] = score_tgt
            val_auc = np.mean([probe[mk][L][sd]["validation_auc"] for sd in CFG.SEEDS])
            tgt_auc = np.mean([probe[mk][L][sd]["target_auc"] for sd in CFG.SEEDS])
            print(f"[S2] {mk} L{L:02d} validation_AUC={val_auc:.3f} target_AUC={tgt_auc:.3f}")
        del Xtr, Xva, Xte

    json.dump(probe, open(PERSIST / "results" / "probe_results.json", "w"), indent=2, default=str)
    yte = np.load(FE[("base", "tgt")][1])
    best = {
        mk: max(probe[mk], key=lambda L: np.mean([
            probe[mk][L][sd]["validation_auc"] for sd in CFG.SEEDS
        ])) for mk in probe
    }
    print("[selection] best layers selected ONLY by source validation AUC:")
    for mk in best:
        L = best[mk]
        va = np.mean([probe[mk][L][sd]["validation_auc"] for sd in CFG.SEEDS])
        tg = np.mean([probe[mk][L][sd]["target_auc"] for sd in CFG.SEEDS])
        print(f"  {mk}: layer {L} | validation AUC={va:.3f} | final target AUC={tg:.3f}")

    fig, ax = plt.subplots(1, 2, figsize=(12, 4.2), sharey=True)
    for j, mk in enumerate(["base", "xlsr"]):
        Ls = sorted(probe[mk])
        for key, color, label in [("validation_auc", "tab:blue", "source validation"),
                                  ("target_auc", "tab:red", "held-out target")]:
            mu = [np.mean([probe[mk][L][sd][key] for sd in CFG.SEEDS]) for L in Ls]
            sd_ = [np.std([probe[mk][L][sd][key] for sd in CFG.SEEDS]) for L in Ls]
            ax[j].plot(Ls, mu, "-o", ms=3, color=color, label=label)
            ax[j].fill_between(Ls, np.array(mu)-sd_, np.array(mu)+sd_, color=color, alpha=0.2)
        ax[j].axvline(best[mk], color="tab:green", ls="--", lw=1.2, label="selected layer")
        ax[j].axhline(0.5, ls="--", c="k", lw=0.8)
        ax[j].set_title(f"{mk}: select L{best[mk]} on validation")
        ax[j].set_xlabel("hidden-state layer"); ax[j].grid(alpha=0.3)
    ax[0].set_ylabel("AUC"); ax[0].legend()
    fig.suptitle("Layer-wise frozen SSL probes: validation-only selection")
    fig.savefig(PERSIST / "figures" / "fig2_layerwise_validation_selected.png", dpi=200, bbox_inches="tight")
    plt.close(fig)

    # Export heads fitted on source training data at validation-selected layers.
    for mk, hf in CFG.SSL_MODELS.items():
        Xtr = np.load(FE[(mk, "src_tr")][0]).astype(np.float32)
        ytr = np.load(FE[(mk, "src_tr")][1])
        set_seed(CFG.SEEDS[0])
        scaler = StandardScaler().fit(Xtr[:, best[mk]])
        clf = LogisticRegression(max_iter=2000, random_state=CFG.SEEDS[0], class_weight="balanced")
        clf.fit(scaler.transform(Xtr[:, best[mk]]), ytr)
        save_probe_head(PERSIST / "ckpts" / f"probe_head_audio_{mk}.npz", scaler, clf,
                        {"backbone": hf, "layer": int(best[mk]), "window_s": CFG.CROP_SEC,
                         "trained_on": CFG.SOURCE_NAME,
                         "selection_protocol": "source_validation_auc_only"})
        del Xtr

    np.save(PERSIST / "results" / "target_labels.npy", yte)
    for mk in best:
        np.save(PERSIST / "results" / f"target_scores_{mk}_L{best[mk]:02d}.npy",
                bank_target[(mk, best[mk], CFG.SEEDS[0])])
    sig = None
    if {"base", "xlsr"} <= set(best):
        sig = paired_bootstrap_delta(yte, bank_target[("base", best["base"], CFG.SEEDS[0])],
                                     bank_target[("xlsr", best["xlsr"], CFG.SEEDS[0])])
        json.dump(sig, open(PERSIST / "results" / "significance_base_vs_xlsr.json", "w"), indent=2)
    json.dump({"selection_protocol": "source_validation_auc_only",
               "selected_layers": {mk: int(best[mk]) for mk in best},
               "per_layer_metrics": probe},
              open(PERSIST / "results" / "layer_selection_protocol.json", "w"),
    ...
    headline = {}
    for mk in best:
        s0 = bank_target[(mk, best[mk], CFG.SEEDS[0])]
