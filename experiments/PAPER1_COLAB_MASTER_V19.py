...
DATASETS = {
    ...
    # ---------- video (PAPER1) ----------
    "celebdf_v2":    {"enabled": True,  "kind": "kaggle", "stages": ("video_train", "video_probe", "cross_gen_df40"),
                      "ref": "reubensuju/celeb-df-v2",
                      "dest": "celebdf-v2"},
    "ffpp_c23":      {"enabled": True,  "kind": "kaggle", "stages": ("video_train", "video_probe", "cross_gen_df40"),
                      "ref": "xdxd003/ff-c23",
                      "dest": "ffpp"},
    # ---------- V19: NEW datasets ---------------------------------------------
...
DATA_EXTS = (".flac", ".wav", ".mp3", ".m4a", ".ogg", ".mp4", ".avi", ".mov", ".csv", ".txt", ".json")
...
def hf_download(ref, dest):
    ...
    try:
        snapshot_download(ref, repo_type="dataset", local_dir=str(d),
...
for name, spec in DATASETS.items():
    ...
    KIND_FN[spec["kind"]](spec["ref"], spec["dest"])
...
if STAGE == "hiercon_audio":
    # ---- Sanity: features from week1_audio must already exist ---------------
    feat_dir = PERSIST / "features"
    xlsr_tr = sorted(feat_dir.glob("xlsr__src_tr__n*.npy"))
    xlsr_va = sorted(feat_dir.glob("xlsr__src_va__n*.npy"))
    xlsr_tg = sorted(feat_dir.glob("xlsr__tgt__n*.npy"))
    assert xlsr_tr and xlsr_va and xlsr_tg, (
        "[hiercon_audio] REFUSING to run — no XLS-R feature cache found. "
        "Run STAGE='week1_audio' first (this stage piggybacks on its features).")
    # Prefer the most-recent (largest N) fingerprint if multiple exist.
    def _pick_latest(paths):
        return max(paths, key=lambda p: (int(p.stem.split("__n")[-1].split("_")[0]),
                                        p.stat().st_mtime))
    fx_tr, fx_va, fx_tg = _pick_latest(xlsr_tr), _pick_latest(xlsr_va), _pick_latest(xlsr_tg)
    ...
    for f in (fy_tr, fy_va, fy_tg):
        assert f.exists(), f"[hiercon_audio] label file missing: {f.name}"
    print(f"[hiercon_audio] reusing week1_audio caches:")
    ...
    class HC:
        ...
        CTR_MARGIN    = 0.25        # AAM-style; keep <0.35 or bonafide cluster collapses
        # eval / logging
    ...
    def train_one_seed(seed):
        ...
        print(f"[seed {seed}] trainable params = {n_params/1e6:.2f} M")
    ...
    all_target_scores = []
    for sd in HC.SEEDS:
        tgt, hist = train_one_seed(sd)
        results[sd] = {"target_ITW": tgt, "history": hist}
    ...
    headline = {
        ...
        "head": "HierCon-lite (intra+inter group attn, contrastive)",
...
if STAGE == "cross_gen_df40":
    ...
    # stratify fakes by method with per-method cap
    by_method = {}
    for t in fakes:
    ...
    fakes_sub = fakes_sub[:DF40.MAX_PER_CLASS]
    ...
    FAMILY = {
        ...
        "FR": {"fomm","facevid2vid","wav2lip","mra","hyperreenact","onlysample",
               "styleface","sadtalker","styleheat","heygen","codeformer"},
...
if STAGE == "eval_dfe2024_v2":
    ...
    def load_labels(root, kind_hint=None):
        ...
        for c in sorted(root.rglob("*.csv")):
            ...
            lcol = next((cols[k] for k in cols
                         if any(t in k for t in ("label","truth","fake","class"))), None)
        ...
        print(f"[dfe2024:{kind_hint}] labels from {src.name}  ({len(df)} rows)")
        lab = df[lcol].astype(str).str.lower().str.strip()
    ...
    if audio_labels:
        ...
        if not audio_head.exists():
            ...
            @torch.no_grad()
            def audio_score_all(pairs, batch=8):
                X_feats, Y, keep_idx = [], [], []
                buf = []
                def flush():
                    if not buf: return
                    ...
                    X_feats.append(hs.mean(1).cpu().numpy().astype(np.float32))
                    Y.extend(b[1] for b in buf); buf.clear()
                for i, (p, y) in enumerate(pairs):
                    try:
                        w, _ = librosa.load(str(p), sr=SR, mono=True)
                        T = int(SR * WIN_S)
                        w = w[(len(w)-T)//2:(len(w)-T)//2+T] if len(w) >= T else np.pad(w, (0, T-len(w)))
                        m = np.abs(w).max(); w = w/m if m > 0 else w
                    except Exception:
                        continue
                    buf.append((w.astype(np.float32), y))
                    keep_idx.append(i)
                    if len(buf) == batch: flush()
                    if (i+1) % 500 == 0: print(f"  [dfe2024:audio] {i+1}/{len(pairs)}")
                flush()
                if not X_feats:
                    return None, None
                X_all = np.concatenate(X_feats)
                s = apply_probe_head(head, X_all)
                return s, np.array(Y)

            s_aud, y_aud = audio_score_all(aud_pairs)
            del w2v; torch.cuda.empty_cache()
            if s_aud is not None and len(np.unique(y_aud)) == 2:
                auc_aud = float(roc_auc_score(y_aud, s_aud))
                eer_aud, _ = eer_of(y_aud, s_aud)
                auc_ci = bootstrap_auc_ci(y_aud, s_aud)
                print(f"[dfe2024:audio] EXPORTED FROZEN HEAD (V19):  AUC {auc_aud:.3f} "
        ...
        if not image_head.exists():
            ...
            def image_score_all(pairs, batch=16):
                ...
                for i, (p, y) in enumerate(pairs):
                    ...
                    v = ((img.astype(np.float32)/255. - CLIP_M2) / CLIP_S2).transpose(2, 0, 1)
                    buf.append((v, y))
                    if len(buf) == batch: flush()
            ...
            if X_img is not None and len(np.unique(y_img)) == 2:
                ...
                auc_img = float(roc_auc_score(y_img, s_img))
                eer_img, _ = eer_of(y_img, s_img)
                auc_ci = bootstrap_auc_ci(y_img, s_img)
                print(f"[dfe2024:image] EXPORTED FROZEN HEAD (V19):  AUC {auc_img:.3f} "
